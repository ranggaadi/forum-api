...
describe('UserRepositoryPostgres', () => {
  ...
  describe('getUsernameById', () => {
    it('should throw invariant error if id provided isnt valid', async () => {
      const userRepo = new UserRepositoryPostgres(pool, {});

      await expect(userRepo.getUsernameById('user-123')).rejects.toThrowError(InvariantError);
    });
    it('should return username of an user by user id', async () => {
      await UsersTableTestHelper.addUser({ id: 'user-123', username: 'simsalabim' });

      const userRepo = new UserRepositoryPostgres(pool, {});

      const username = await userRepo.getUsernameById('user-123');

      expect(username).toEqual('simsalabim');
    });
  });
});
